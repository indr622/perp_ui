  ...
  <div>
    ...
    <v-simple-table class="mx-2 mb-3" fixed-header>
      ...
      <tbody>
        <tr>
          <td>
            <v-textarea label="Term And Condition" dense outlined rows="3" hide-details></v-textarea>
          </td>
          <td></td>
          <td class="text-right">
            Sub Total : <br />
          ...
          <td>
            ...
            <hr />
            <b>{{ this.Quotation.additional_cost | currency("", 2, { symbolOnLeft: false }) }} </b>
            <hr />
...
export default {
  ...
  computed: {
    ...
    sub_total() {
      return _.sumBy(this.Quotation.product_list, function (t) {
        return (t.price_sell * t.qty) / 100
      });
    },
    discount() {
      if (this.Quotation.discount_percent > 100 || this.Quotation.discount_percent < 0) {
        this.SET_DISCOUNT(0);
        return "Discount not valid value";
      }
      this.SET_DISCOUNT(
        (this.sub_total * this.Quotation.discount_percent) / 100
      );
      return (this.sub_total * this.Quotation.discount_percent) / 100;
    },
    afterDiscount() {
