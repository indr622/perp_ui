 <template>
  <div>
    <base-title title="Production Step" subtitle="List Data Production Step" />
    ...
    >
      ...
      <template v-slot:top>
        <v-toolbar flat color="white">
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <v-dialog v-model="dialogForm" max-width="350px" persistent>
...
export default {
  ...
  data: () => ({
    ...
    rule_name: [(v) => !!v || "Production step name please fill in !!!"],
  }),
  ...
  methods: {
    initialize: async function () {
      ...
      await this.$axios
        ...
        .then(({ data }) => {
          this.datatable = data.production_step;
    ...
    save: async function () {
      if (this.$refs.form_data.validate()) {
        ...
        if (this.editedIndex > -1) {
          await this.$axios
            .post(
              "/production-step/" + this.form_data.id,
              {
                _method: "PUT",
          ...
          await this.$axios
            ...
            .then(({ data }) => {
              this.datatable.push(data.production_step);
              this.closedialogForm();
              this.btnLoading = false;
            })
            .catch(() => {
