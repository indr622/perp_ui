<template>
  <div>
    <base-title title="Term Payment" subtitle="List Data Term Payment" />
    <v-data-table
      :headers="headers"
      :items="datatable"
      :search="search"
...
export default {
  ...
  data: () => ({
    headers: [
      { text: "Term Payment Name", value: "name" },
  ...
  methods: {
    initialize: async function () {
      ...
      await this.$axios
        ...
        .then(({ data }) => {
          this.datatable = data.term_payment;
    ...
    save: async function () {
      if (this.$refs.form_data.validate()) {
        ...
        if (this.editedIndex > -1) {
          await this.$axios
            .post(
              "/term-payment/" + this.form_data.id,
              {
                _method: "PUT",
          ...
          await this.$axios
            ...
            .then(({ data }) => {
              this.datatable.push(data.term_payment);
              this.closedialogForm();
              this.btnLoading = false;
            })
            .catch(() => {
