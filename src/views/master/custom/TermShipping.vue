<template>
  <div>
    <base-title title="Term Shipping" subtitle="List Data Term Shipping" />
    <v-data-table
      :headers="headers"
      :items="datatable"
      :search="search"
...
export default {
  ...
  data: () => ({
    headers: [
      { text: "Term Shipping Name", value: "name" },
  ...
  methods: {
    initialize: async function () {
      ...
      await this.$axios
        ...
        .then(({ data }) => {
          this.datatable = data.term_shipping;
    ...
    save: async function () {
      if (this.$refs.form_data.validate()) {
        ...
        if (this.editedIndex > -1) {
          await this.$axios
            .post(
              "/term-shipping/" + this.form_data.id,
              {
                _method: "PUT",
          ...
          await this.$axios
            ...
            .then(({ data }) => {
              this.datatable.push(data.term_shipping);
              this.closedialogForm();
              this.btnLoading = false;
            })
            .catch(() => {
